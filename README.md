...
Panchang uses `lahiri_upper_limb_v1`: Lahiri sidereal, upper solar limb with refraction, sea-level observer, 1013.25 hPa and 15°C. Its day is `[sunrise, next_sunrise)`. Each limb array begins with the value at sunrise and includes subsequent transitions; the last `ends_at` may extend beyond next sunrise. Timestamps include UTC offsets (UTC `Z` for historical subminute offsets). Root-search tolerance is 0.1 seconds and output is rounded to seconds; these are numerical tolerances, not accuracy guarantees. No supplied-instant option exists; clients can select the segment containing an instant inside the covered day.
